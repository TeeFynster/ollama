...
func (h *Hermes) ensureInstalledFor(command string) error {
	...
	if hermesGOOS == "windows" {
		return hermesWindowsHint(command)
	}
...
func (h *Hermes) binary() (string, error) {
	...
	if hermesGOOS == "windows" {
		return "", hermesWindowsHint("hermes")
	}
...
func hermesWindowsHint(command string) error {
	return fmt.Errorf("Hermes on Windows requires WSL2. Install WSL with: wsl --install\n"+
		"Then run 'ollama launch %s' from inside your WSL shell.\n"+
		"Docs: https://hermes-agent.nousresearch.com/docs/getting-started/installation/", command)
}
